...
public struct DefaultLogMessageFormatter: LogMessageFormatter {
    ...
    public func formatMessage(_ logMessage: LogMessage) -> String {
        ...
        let thread = Thread.isMainThread ? "main" : Thread.current.name ?? "thread-\(Thread.current.hash)"

        return "\(timestamp) [\(logMessage.level.stringValue)] [\(thread)] \(fileName):\(logMessage.line) - \(logMessage.function) > \(logMessage.message)"
    }
}
